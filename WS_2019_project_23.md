...
## Task 7: Implement a simple application
We decided to create a simple command line interface application. We used python with the "argparse" package to do so. In the README attached there is a step by step guide how to use the application with ones personal data. This can also be skipped by just using the application with our data provided, which is already transformed into RDF. The application parses the RDF file (which is given as an argument in the command line) and queries the data. It either shows the songs the user listened to from a certain artist (or from all artists) or how long the user listened to one artist (or all artists). The application also shows a help page with the argument "-h" to get a good description of the possible arguments.
